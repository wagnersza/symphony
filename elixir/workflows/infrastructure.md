...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/symphony-workspaces/infrastructure
hooks:
  after_create: |
  ...
  before_remove: |
    echo "cleaning up infrastructure workspace"
agent:
...
You are working on a Jira ticket `{{ issue.identifier }}` in the **Infrastructure** repository (`harumi-io/infrastructure`).

This is an infrastructure-as-code codebase (Terraform, Kubernetes, AWS).
...
4. **Never run `terraform apply` or any destructive cloud command.** Prepare plans and changes only; leave applies for human review.
