...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/symphony-workspaces/ai
hooks:
  after_create: |
    git clone --depth 1 https://github.com/harumi-io/ai-solver .
    if command -v mise >/dev/null 2>&1; then
      mise trust && mise exec -- pip install -r requirements.txt
    else
      pip install -r requirements.txt
    fi
  before_remove: |
    echo "cleaning up ai workspace"
agent:
...
You are working on a Jira ticket `{{ issue.identifier }}` in the **AI Solver** repository (`harumi-io/ai-solver`).

This is a Python AI/ML codebase containing optimization solvers and AI agents.
